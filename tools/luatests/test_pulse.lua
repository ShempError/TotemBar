...
H.run("ringFrameIndex: 0=empty .. frameCount-1=full (Rev 2: 62-frame arc)", function()
    ...
    -- Regression (2026-07-16 Searing-ring trace, /tb tdump measurement:
    -- own=36.263/50, GTI absent): pins the exact frame the trace proved
    -- correct offline (44, strictly between empty=0 and full=61) - the
    -- bug the render-state dump above targets is NOT in this math.
    H.assert_eq(TotemBar.ringFrameIndex(36.263, 50, 62), 44, "Searing trace: 72% remaining -> frame 44 (0 < 44 < 61)")
end)
...
H.run("resolveDuration: mirrors resolveRemaining precedence", function()
    ...
    H.assert_eq(TotemBar.resolveDuration(true, 0, 45, 40, 20), 20, "GTI active but expired -> own duration fallback (2026-07-15 fix)")
    ...
    -- Regression (2026-07-16 Searing-ring trace, /tb tdump measurement:
    -- own=36.263/50, GTI absent/nil): pins the exact own-tracking fallback
    -- duration the trace proved correct offline for the "text shows, no
    -- ring" report - the bug the render-state dump above targets is NOT
    -- in this math.
    H.assert_eq(TotemBar.resolveDuration(nil, nil, nil, 36.263, 50), 50, "Searing trace: GTI absent, own 36.263/50 -> 50")
end)

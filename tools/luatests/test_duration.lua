...
H.run("resolveRemaining: GTI active but out of time falls back to own tracking (2026-07-15 fix)", function()
    -- Stale-active GTI slot (active but no positive remaining -- libtotem evicts lazily on read)
    -- must NOT block display when own-tracking still has a valid time (was the Fire/Earth-dead bug).
    H.assert_eq(TotemBar.resolveRemaining(true, 0, 40), 40, "GTI active, 0 remaining -> own fallback")
    H.assert_eq(TotemBar.resolveRemaining(true, nil, 40), 40, "GTI active, no timing -> own fallback")
    H.assert_eq(TotemBar.resolveRemaining(true, 0, nil), nil, "GTI active, 0 remaining, no own record -> nil")
end)

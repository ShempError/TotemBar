...
H.run("ensureDefaults: fills new fields on a fresh DB", function()
    ...
    -- The DropSet button is visible unless explicitly turned off, so upgrading
    -- users never lose it.
    H.assert_eq(TotemBarDB.showDropSetButton, true, "showDropSetButton default true")
end)
...
H.run("ensureDefaults: preserves existing values", function()
    ...
    -- false must survive - the "== nil" default must not read it as unset.
    H.assert_eq(TotemBarDB.showDropSetButton, false, "showDropSetButton false preserved")
end)
